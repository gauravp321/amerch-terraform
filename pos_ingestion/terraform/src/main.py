...
def _load_secrets(project_id: Optional[str] = None) -> None:
    ...
    if not project_id:
        try:
            ...
            raise SecretError(f"Failed to get project ID from default credentials: {str(e)}")
...
def emit_metric(project_id: str, metric_type: str, value: int, labels: Dict[str, str] = None):
    """
    Emit a custom metric to Cloud Monitoring.
    Uses generic_task resource type for Cloud Functions.
    """
    if not project_id:
        return
    ...
    try:
        ...
        series.resource.labels["location"] = os.environ.get("FUNCTION_REGION", "us-central1")
        series.resource.labels["namespace"] = "cloud_functions"
        series.resource.labels["job"] = os.environ.get("FUNCTION_NAME", "gcf_pos_file")
        series.resource.labels["task_id"] = os.environ.get("FUNCTION_TARGET", "process_config_file")
        ...
        logger.warning(f"Failed to emit metric {metric_type}: {str(e)}")
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    if not email:
        raise ConfigValidationError("Email is required and cannot be empty.")
    ...
    dataset_id = config.get('dataset', 'pos_files')
...
def load_data_to_bigquery(
    ...
    try:
        ...
        if load_job.output_rows == 0:
            logger.warning("Zero rows loaded into BigQuery.")
        
        if bad_records > BAD_RECORD_THRESHOLD:
            raise DataQualityError(f"Too many bad records: {bad_records} (Threshold: {BAD_RECORD_THRESHOLD})")
...
def send_email_notifications(to_email: list, subject: str, content: str, is_error: bool = False, project_id: Optional[str] = None) -> None:
    ...
    try:
        ...
        logger.error(f"Secret validation failed from Secret Manager: {str(e)}")
    ...
    try:
        ...
        logger.error(
            f"Failed to send email. Subject: '{subject}', Recipients: {to_email}. "
            f"Error: {str(e)}", 
            exc_info=True
        )
...
def process_config_file(cloud_event):
    ...
    try:
        ...
        data_blob.reload()
        data_file_size_bytes = data_blob.size
        ...
        try:
            ...
            try:
                if not email_list:
                    logger.error(f"No recipient email found. Cannot send cleanup failure warning.")
                else:
            ...
            emit_metric(project_id, "pos_ingestion/duration", duration, {"status": "failure", "table": table_name})        
            emit_metric(project_id, "pos_ingestion/execution_count", 1, {"status": "failure", "table": table_name})
            emit_metric(project_id, "pos_ingestion/error_count", 1, {"error_type": error_type, "table": table_name})
        ...
        emit_metric(project_id, "pos_ingestion/duration", duration, {"status": "success", "table": table_name})
        emit_metric(project_id, "pos_ingestion/rows_loaded", rows_loaded, {"table": table_name})
        emit_metric(project_id, "pos_ingestion/file_size_bytes", data_file_size_bytes, {"table": table_name})        
        emit_metric(project_id, "pos_ingestion/execution_count", 1, {"status": "success", "table": table_name})
        ...
        return 'OK'
            ...
            TableConflictError, InvalidCSVFormatError, MemoryLimitError, GoogleAuthError, FileNotFoundError, ConnectionError, SecretError, DataQualityError) as e:
        error_msg = str(e)
        ...
        try:
            if not email_list:
                logger.error(f"No recipient email to send failure notification.")
            else:
                send_email_notifications(email_list, subject, body, is_error=True, project_id=project_id)
        except Exception as email_e:
